...
var settings = [
	...
	{ 
		...
		"def": 0.2,
		"mobile_def": 0.32,
		"range": { "min": 0.025, "max": 0.6 },
		"desc": "Pull slider to the right to make moving/rotating take longer"
	},
...
func check_config():
	...
	for i in range(settings.size()):
		...
		if not config.has_section_key(s.sec, s.name):
			var default = s.def
			if s.has('mobile_def') and GlobalHelper.is_mobile():
				default = s.mobile_def
			
			config.set_value(s.sec, s.name, default)
